...
export REACT_NATIVE_CORE_VERSION=`node -e 'console.log(require("./node_modules/react-native-core/package.json").version)'`
...
console.log(`Artifacts are now in $HERMES_TV_ARCHIVE_DIR`);
console.log('You can publish them to the local (/tmp/maven-local) repository:');
console.log('  ./gradlew :ReactAndroid:external-artifacts:publishAllPublicationsToMavenTempLocalRepository');
